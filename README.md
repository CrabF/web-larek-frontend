...
3. Класс **Basket** - требуется для представления корзины. Корзина должна принимать массив с карточками товаров, а также общую стоимость товаров. Класс типизирован *IBasket*. Класс расширяет базовый **Component** передавая тому *IBasket* - массив с разметкой карточек и общую стоимость  карточек.
...
1. Сеттер *items* - требуется для для вставки информации в элемент разметки отвечающий за отображение карточек товаров. Аргументом метода является *value* типа *HTMLElement[]*. 
...
*payment:changed* - вызывается при изменении способа оплаты
